...
void RETRO_BlitRect(RECT src_rect, unsigned char *src_buf, int src_pitch, RECT dest_rect, unsigned char *dest_buf, int dest_pitch, int alpha = -1)
{
	...
	for (int y = 0; y < dest_ydiff; y++) {
		for (int x = 0; x < dest_xdiff; x++) {
			...
			if (dest_x >= 0 && dest_x < RETRO_WIDTH && dest_y >= 0 && dest_y < RETRO_HEIGHT) {
				if (alpha == -1 || src_buf[src_y * src_pitch + src_x] != alpha) {
					dest_buf[dest_y * dest_pitch + dest_x] = src_buf[src_y * src_pitch + src_x];
				}
			}
		}
	}
