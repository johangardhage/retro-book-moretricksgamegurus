  ...
  while(1)
  {
    ...
    if (!memcmp(chunk.ckID, BODY, BYTE_LEN) )
    {
      ...
      for (ilbmRow=0; ilbmRow<ilbmHeight; ilbmRow++)
      {
            ...
            // unPacker
            while(ilbmByteIndex) // loop until the desired number of bytes 
            {
              if (ilbmHeader.compression)
              {
                ilbmByte = fgetc(fp);
                if (ilbmByte == 128) // no operation
                  continue;
                if (ilbmByte > 0) // copy the next n+1 bytes literally
                {
                  int len;
                  len = ilbmByte + 1;
                  ilbmByteIndex -= len;
            ...
            _fmemcpy( lbmBuf + offset, scanlineBuf, ilbmByteCount);
      ...
      for (ilbmRow=0; ilbmRow<ilbmHeight; ilbmRow++)
      {
            ...
            while(ilbmByteIndex) // loop until the desired number of bytes 
            {
              if (ilbmHeader.compression)
              {
                ...
                else  // replicate the next byte -n + 1 times
                {
                  int count;
                  count = -ilbmByte;
                  count++;
            ...
            _fmemcpy( p.pixBuf + offset, scanlineBuf, ilbmByteCount);
           offset += ilbmByteCount;
      }// for ilbmRow
      break;
    }  // if BODY
