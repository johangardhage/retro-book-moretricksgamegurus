...
void main(int argc, char **argv)
{
	...
	fread(sound, (size_t)BUFFSIZE, 1, fp);	//if file's len < BUFFSIZE, this works
	...
	*/
	...
	/*
	 . Set Master Volume to about 4/5 th's of max
	*/
	if (!dws_XMaster(200))
	{
		DisplayError(dws_ErrNo());
	...
	if (!dws_DGetRateFromDWD(dplay.snd, &ideal.digrate))
	{
		DisplayError(dws_ErrNo());
		goto KillIt;
	}

	if (!dws_DSetRate(ideal.digrate))
	{
		DisplayError(dws_ErrNo());
		goto KillIt;
	}
